...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['comment_text'])) {
  $movieId = $_POST['movie_id']; // Get the movie ID from the form
  $comment = $_POST['comment'];
  
  // Prepare and bind
  $stmt = $conn->prepare("INSERT INTO comments (comment_text, movie_id) VALUES (?, ?)");
  $stmt->bind_param("si", $comment, $movieId); // 'si' means string, integer
  
  if ($stmt->execute()) {
      echo "Comment added successfully!";
  } else {
      echo "Error submitting the comment: " . $stmt->error;
  }
  $stmt->close();
  exit();
}

// Assume $currentMovieId holds the ID of the current movie being viewed
$currentMovieId = isset($_GET['id']) ? (int)$_GET['id'] : 0;
$sql = "SELECT comment_text, created_at FROM comments WHERE movie_id = ? ORDER BY created_at DESC";

// Prepare and bind
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $currentMovieId);
$stmt->execute();
...
if ($result->num_rows > 0) {
    ...
    echo "<p>No comments found for this movie.</p>";
}
  ...
  <div class="modal" id="movieModal">
    <div class="modal-content">
      ...
      <div class="comment-section">
        <h3>Comments</h3>
        <<form id="commentForm">
  <input type="hidden" id="movieId" value="<!-- PHP code to echo current movie ID -->" />
  <textarea id="comment" rows="3" placeholder="Add a comment..." required></textarea>
...
<script>
    ...
    .then(data => {
        ...
        fetchComments(); // Optionally, fetch the comments again to display
    })
    .catch(error => console.error('Error:', error));
});

function fetchComments() {
    fetch('indexx.php?fetch_comments=true')
        .then(response => response.text())
        .then(data => {
            document.getElementById('commentsList').innerHTML = data; // Display comments
        });
}
// Call fetchComments on page load to load existing comments
fetchComments();
</script>
</body>
</html>
